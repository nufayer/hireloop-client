...
const CompanyTable = ({ companies }) => {
    ...
    const handleUpdateStatus = async (id, status) => {
        setUpdatingId(id);
        try {
            const result = await updateCompany(id, { status });
            if (result.acknowledged) {
                console.log(`Successfully updated status to ${status} for company ${id}`);
                router.refresh();
            } else {
                                            ...
                                            <div className="flex justify-end gap-2">
                                                {company.status?.toLowerCase() !== 'approved' && (
                                                    <Button
                                                        size="sm"
                                                        variant="light"
                                                        isLoading={updatingId === companyId}
                                                        isDisabled={!!updatingId}
                                                        onClick={() => handleUpdateStatus(companyId, 'Approved')}
                                                ...
                                                {company.status?.toLowerCase() !== 'rejected' && (
                                                    <Button
                                                        ...
                                                        isLoading={updatingId === companyId}
                                                        isDisabled={!!updatingId}
                                                        onClick={() => handleUpdateStatus(companyId, 'Rejected')}
